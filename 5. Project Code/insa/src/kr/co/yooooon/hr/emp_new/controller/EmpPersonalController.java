...
public class EmpPersonalController extends MultiActionController {
	private static EmpServiceFacade empServiceFacade = EmpServiceFacadeImpl.getInstance();
	PrintWriter out = null;

	public ModelAndView empPersonal(HttpServletRequest request, HttpServletResponse response) {
		response.setContentType("application/json; charset=UTF-8");
		HashMap<String, Object> map = new HashMap<String, Object>();
		String empCode = request.getParameter("code");
		//System.out.println("@@"+empCode);
		try {
			ArrayList<EmpInforTO> list = (ArrayList<EmpInforTO>) empServiceFacade.empPersonal(empCode);
			map.put("list", list);
			ArrayList<EmpResidentTO> list2 = (ArrayList<EmpResidentTO>) empServiceFacade.findEmpResident(empCode);
			map.put("list2", list2);
